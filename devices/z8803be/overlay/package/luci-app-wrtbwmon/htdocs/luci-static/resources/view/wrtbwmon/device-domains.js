...
var callGetDeviceDomains = rpc.declare({
	object: 'luci.wrtbwmon',
	method: 'get_device_domains',
	params: ['mac', 'date_filter', 'limit']
});
...
function getMacFromUrl() {
	// Try URL query parameter first (new method)
	var params = new URLSearchParams(window.location.search);
	var mac = params.get('mac');
	if (mac) return decodeURIComponent(mac);

	// Fallback to hash for backward compatibility
	var hash = window.location.hash;
	var match = hash.match(/#device-domains\/([^\/]+)/);
...
return view.extend({
	load: function() {
		...
		var mac = getMacFromUrl();
		if (!mac) {
			return Promise.reject('No MAC address specified');
		}
		return Promise.all([
			callGetDeviceInfo(mac),
			callGetDeviceDomains(mac, 'today', 1000)
		]);
	},
	...
	render: function(data) {
		...
		var filterButtons = Object.keys(filterLabels).map(function(filter) {
			return E('button', {
				...
				'click': function() {
					...
					callGetDeviceDomains(mac, filter, 1000).then(function(result) {
		...
		var tableBody = E('tbody', { 'id': 'device-domain-table-body' });
		...
		function refreshData() {
			callGetDeviceDomains(mac, currentFilter, 1000).then(function(result) {
				domains = result.domains || [];
				warning = result.warning || '';
				updateTable(domains);
		...
		return E('div', { 'class': 'cbi-map zbt-app zbt-traffic' }, [
			E('div', { 'class': 'device-header' }, [
				E('button', {
					...
					'click': function() {
						window.location.href = L.url('admin/services/traffic/details');
					}
				}, _('← Back to Devices')),
				E('h2', {}, _('Top 1000 Domains - ') + (deviceInfo.hostname || mac))
			]),
			labelUpdated,
			warningBox,
